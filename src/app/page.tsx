...
export default function WeatherPage() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-gradient-to-br from-background to-secondary/30 dark:from-background dark:to-muted/20">
      <Navbar />
      <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex flex-col items-center overflow-y-auto">
        <section className="w-full max-w-xl mb-6 text-center">
        ...
        {isPending && (
          <div className="flex flex-col items-center justify-center space-y-4 mt-8 p-6 bg-card/50 backdrop-blur-sm rounded-xl shadow-lg">
            <svg className="animate-spin h-10 w-10 text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="text-md text-muted-foreground font-medium">Fetching weather data...</p>
          </div>
        )}
        ...
        {showInitialMessage && !state.data && !isPending && (
            <Card className="w-full max-w-lg mt-6 bg-card/80 backdrop-blur-md shadow-xl border border-primary/20 transform hover:scale-[1.01] transition-transform duration-300">
                <CardHeader className="items-center text-center pt-6 pb-4">
                    <CloudSun className="h-16 w-16 text-primary mb-3 drop-shadow-lg" />
                ...
                <CardContent className="flex justify-center pb-6 px-6">
                    <Image src="https://placehold.co/380x230.png" alt="Weather illustration" width={380} height={230} className="rounded-xl shadow-lg border border-border/20" data-ai-hint="weather forecast app"/>
                </CardContent>
            </Card>
        )}

        {state.cityNotFound && !isPending && (
             <Card className="w-full max-w-lg mt-6 border-destructive/50 bg-destructive/10 backdrop-blur-sm shadow-xl transform hover:scale-[1.01] transition-transform duration-300">
                <CardHeader className="items-center text-center pt-6 pb-4">
                    <AlertCircle className="h-16 w-16 text-destructive mb-3 drop-shadow-lg" />
                    <CardTitle className="text-2xl font-headline text-destructive">City Not Found</CardTitle>
                     <CardDescription className="text-base text-destructive/90 mt-1 px-4">
                        We couldn't find the city you searched for. Please check the spelling or try a different city.
                    </CardDescription>
                </CardHeader>
                <CardContent className="flex justify-center pb-6 px-6">
                     <Image src="https://placehold.co/330x180.png" alt="Map indicating not found" width={330} height={180} className="rounded-xl mt-2 opacity-90 shadow-lg border border-border/20" data-ai-hint="map error"/>
                </CardContent>
            </Card>
        )}
      </main>
